...
class LLaVATrainer(Trainer):
    ...
    def create_accelerator_and_postprocess(self):
        ...
        # NOTE: `dispatch_batches` / `split_batches` are not available in some
        # accelerate versions, so we add them only when supported.
        accelerator_init_params = inspect.signature(Accelerator.__init__).parameters
        accelerator_kwargs = dict(
            deepspeed_plugin=self.args.deepspeed_plugin,
            gradient_accumulation_plugin=gradient_accumulation_plugin,
            kwargs_handlers=[accelerator_kwargs],
        )
        if "dispatch_batches" in accelerator_init_params:
            accelerator_kwargs["dispatch_batches"] = getattr(self.args, "dispatch_batches", None)
        if "split_batches" in accelerator_init_params:
            accelerator_kwargs["split_batches"] = getattr(self.args, "split_batches", None)
