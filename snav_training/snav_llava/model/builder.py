...
from snav_llava.constants import DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
from snav_llava.utils import rank0_print


def _infer_vocab_size_from_weights(model_path):
    ...
    for tensor_name in candidate_names:
        ...
        with safe_open(model_path / shard_name, framework="pt", device="cpu") as handle:
            return handle.get_slice(tensor_name).get_shape()[0]

    return None


def _expand_tokenizer_to_vocab_size(tokenizer, target_vocab_size):
    missing = target_vocab_size - len(tokenizer)
    if missing <= 0:
        return
    ...
    for token in [DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN]:
    ...
    filler_needed = target_vocab_size - len(tokenizer)
    if filler_needed > 0:
        filler_tokens = [f"<extra_unused_token_{idx}>" for idx in range(filler_needed)]
        tokenizer.add_tokens(filler_tokens, special_tokens=False)

    if special_tokens or filler_needed > 0:
        rank0_print(f"Expanded tokenizer to match checkpoint vocab: {len(tokenizer)}")


def _align_tokenizer_vocab_with_config(tokenizer, config, target_vocab_size=None):
    target_vocab_size = target_vocab_size or len(tokenizer)
    changed = False

    if getattr(config, "vocab_size", None) != target_vocab_size:
        config.vocab_size = target_vocab_size
        changed = True

    text_config = getattr(config, "text_config", None)
    if isinstance(text_config, dict):
        if text_config.get("vocab_size") != target_vocab_size:
            text_config["vocab_size"] = target_vocab_size
            changed = True
    elif text_config is not None and getattr(text_config, "vocab_size", None) != target_vocab_size:
        text_config.vocab_size = target_vocab_size
        changed = True

    if changed:
        rank0_print(f"Aligned config vocab_size to tokenizer length: {target_vocab_size}")
    ...
    if "llava" in model_name.lower() or "correctnav" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                from snav_llava.model.language_model.llava_mixtral import LlavaMixtralConfig
                ...
                from snav_llava.model.language_model.llava_mistral import LlavaMistralConfig
                ...
                from snav_llava.model.language_model.llava_gemma import LlavaGemmaConfig
                ...
                from snav_llava.model.language_model.llava_qwen import LlavaQwenConfig
                lora_cfg_pretrained = LlavaQwenConfig.from_pretrained(model_path)
                target_vocab_size = _infer_vocab_size_from_weights(model_path) or getattr(lora_cfg_pretrained, "vocab_size", None)
                if target_vocab_size is not None:
                    _expand_tokenizer_to_vocab_size(tokenizer, target_vocab_size)
                    lora_cfg_pretrained = _align_tokenizer_vocab_with_config(tokenizer, lora_cfg_pretrained, target_vocab_size)
                ...
                from snav_llava.model.language_model.llava_llama import LlavaConfig
            ...
            rank0_print(f"Loaded snav_llava model: {model_path}")
            if "mixtral" in model_name.lower():
                from snav_llava.model.language_model.llava_mixtral import LlavaMixtralConfig
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    from snav_llava.model.language_model.llava_qwen_moe import LlavaQwenMoeConfig
                    if overwrite_config is not None:
                    ...
                    target_vocab_size = _infer_vocab_size_from_weights(model_path) or getattr(llava_cfg, "vocab_size", None)
                    if target_vocab_size is not None:
                        _expand_tokenizer_to_vocab_size(tokenizer, target_vocab_size)
                        llava_cfg = _align_tokenizer_vocab_with_config(tokenizer, llava_cfg, target_vocab_size)
                    if overwrite_config is not None:
                        rank0_print(f"Overwriting config with {overwrite_config}")
                        for k, v in overwrite_config.items():
                            setattr(llava_cfg, k, v)
